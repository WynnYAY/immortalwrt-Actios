...
for cfg in target/linux/msm89xx/config-*; do
  ...
  grep -q 'CONFIG_INET_ESPINTCP_OFFLOAD' "$cfg" || echo '# CONFIG_INET_ESPINTCP_OFFLOAD is not set' >> "$cfg"
  grep -q 'CONFIG_INET6_ESP_OFFLOAD' "$cfg" || echo '# CONFIG_INET6_ESP_OFFLOAD is not set' >> "$cfg"
  grep -q 'CONFIG_INET6_ESPINTCP_OFFLOAD' "$cfg" || echo '# CONFIG_INET6_ESPINTCP_OFFLOAD is not set' >> "$cfg"
